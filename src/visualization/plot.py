...
def plot_pnl_distributions(
    ...
    # Fixed, symmetric window around 0 (was asymmetric -2.0..1.5)
    x_min, x_max = -1.5, 1.5
    ...
    ax.set_xlabel(r"Normalized Terminal P\&L", fontsize=FONT_LABEL)
    ax.set_ylabel("Density", fontsize=FONT_LABEL)

    # More granular x-axis ticks: major every 0.25, minor every 0.05
...
def plot_scoring_curvature(
    ...
    for ax, xlabel in panel_xlabels:
        ...
        ax.set_ylabel(r"$\mathfrak{s}(\mathfrak{a}_1,\mathfrak{a}_2)$", fontsize=FONT_LABEL, color="#444444")
        _style_ax(ax)
...
def plot_static_risk_across_maturities(
    ...
    for sk, results in drm_results_by_fn.items():
        ...
        ax.plot(maturities_days, cvar_vals, color=SCORING_COLOR.get(sk, "#888888"),
                linewidth=1.6, alpha=0.85, label=sk)
...
def compute_fixed_price_comparison(
    ...
    def _nested_price_t0(ckpt_path, states_t0, T, B0_accrued):
        ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
        critic = CriticVaR(
            env.state_dim, T,
            hidden_dim=nested_hidden_dim,
    ...
    for alpha_label in alpha_labels:
        ...
        if not os.path.exists(deriv_path):
            ...
            continue
        B0_accrued = float(np.load(deriv_path)[:, 0].mean()) * torch.exp(env.r_daily * 252).item() #without .mean, this variable should be the same value (fixed initial state).
        print(f"average price: {B0_accrued}, path 0 price: {float(np.load(deriv_path)[0, 0])}")
        ...
        if os.path.exists(static_pnl_path):
            ...
            srm_price     = _cvar(static_losses, alpha_val) + B0_accrued
        else:
        ...
        for sk in scoring_keys:
            ...
            drm_price = float(risk_t0.mean().cpu()) + B0_accrued

            nested_price = float("nan")
            nested_key = (alpha_label, sk)
            if nested_key in nested_ckpt_paths:
                T = env.T_days
                nested_price = _nested_price_t0(nested_ckpt_paths[nested_key], states_t0, T, B0_accrued)
