...
We study a deep hedging framework in which the hedging agent minimizes risk through a time-consistent dynamic CVaR objective rather than a static one using an actor-critic RL algorithm. The critic estimates dynamic risk using the conditional elicitability of CVaR, which, in the literature, is shown to improve training efficiency in RL environments. The stochastic policy is updated via policy gradient against the risk estimated by the critic. We apply this framework to a basket option on 4 underlying assets evolving under a DCC-GARCH model across a daily, 1-year time grid, and compare the trained dynamic risk models against a static risk model across 4 risk thresholds (92.5%, 95%, 97.5%, 99%) and 6 scoring function choices under the elicitability approach for the dynamic risk models.
